...
const QuickAuth: React.FC<QuickAuthProps> = ({ initialMode = "auto" }) => {
  ...
  // const [otpChannel, setOtpChannel] = useState<"sms" | "whatsapp">("sms"); // DÉSACTIVÉ
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      router.push(buildVerifyUrl({ type: "password-reset", phone: fullPhone, /* channel: otpChannel, */ attemptsRemaining: data?.data?.attemptsRemaining, cooldownSeconds: data?.data?.cooldownSeconds, expiresInSeconds: data?.data?.expiresInSeconds }));
    } catch (e) { showAlert("error", getErrorMessage(e)); }
    finally { setIsLoading(false); }
  };
    ...
    <div className="min-h-screen flex bg-white">
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-6 sm:px-10 py-12 bg-[#fafbfc]">
        ...
        <div className="w-full max-w-md">
          ...
          {/* Canal OTP — WhatsApp DÉSACTIVÉ (template Meta en attente d'approbation) */}
          {/* {(step === "register" || step === "login") && (
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => setOtpChannel("sms")} ...>SMS</button>
                <button type="button" onClick={() => setOtpChannel("whatsapp")} ...>WhatsApp</button>
              </div>
            </div>
          )} */}
