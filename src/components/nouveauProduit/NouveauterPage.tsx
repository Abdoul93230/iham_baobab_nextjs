...
import { Search, X, SlidersHorizontal, Sparkles, Star } from "lucide-react";
...
export default function NouveauterPage({ acces = true }: NouveauterPageProps) {
  ...
  // Newest products: sort by createdAt desc, take top 60
  const nouveauxProduits: ProductCardData[] = useMemo(() => {
  ...
  // Search suggestions
  useEffect(() => {
    if (search.length >= 2 && nouveauxProduits.length > 0) {
      const s = nouveauxProduits
  ...
  }, [search, nouveauxProduits]);

  const filteredProducts: ProductCardData[] = useMemo(() => {
    let list: any[] = nouveauxProduits;
  ...
  }, [nouveauxProduits, activeCategory, search, sortBy, DATA_Categories, DATA_Types]);

  return (
    <div className="min-h-screen bg-gray-50">
      {/* ── Hero ─────────────────────────────────────────────────────── */}
      <div className="relative bg-gradient-to-br from-[#30A08B] via-[#1d7a6a] to-[#B17236] overflow-hidden">
        {/* Decorative circles */}
        ...
        <div className="relative max-w-5xl mx-auto px-4 py-10 sm:py-14 flex flex-col items-center gap-5 text-center">
          ...
          <p className="text-white/80 text-sm sm:text-base max-w-md">
            {filteredProducts.length} nouveau{filteredProducts.length !== 1 ? "x" : ""} produit
            {filteredProducts.length !== 1 ? "s" : ""} — ajoutés récemment
          </p>
          ...
          <div className="relative w-full max-w-lg">
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
              {search && (
                <button onClick={() => { setSearch(""); setShowSuggestions(false); }} className="text-gray-400 hover:text-gray-600">
                  <X size={14} />
                </button>
              )}
          ...
          <div className="flex flex-wrap justify-center gap-2 text-xs">
            {["🆕 Nouveautés", "🚚 Livraison rapide", "✨ Qualité premium", "🔒 Paiement sécurisé"].map((b) => (
                  ...
                  {SORT_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                        sortBy === opt.value ? "bg-[#30A08B]/10 text-[#30A08B] font-semibold" : "text-gray-700 hover:bg-gray-50"
                      }`}
                    >
                      {opt.label}
                    </button>
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">
                {search ? `Aucun résultat pour "${search}"` : "Aucun nouveau produit disponible"}
              </p>
            </div>
            {(search || activeCategory !== "all") && (
              <button
                onClick={() => { setSearch(""); setActiveCategory("all"); setSortBy("newest"); }}
